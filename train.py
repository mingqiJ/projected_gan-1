...
def init_dataset_kwargs(data):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data, use_labels=True, max_size=None, xflip=False, fname=None)
...
@click.option('--cada_exp_val',  help='class balancing data exp',           metavar='FLOAT', type=click.FloatRange(min=0, max=1), default=0.35)
...
@click.option('--snap',         help='How often to save snapshots', metavar='TICKS',            type=click.IntRange(min=1), default=400, show_default=True)
...
def main(**kwargs):
    ...
    c.cada_exp_val = opts.cada_exp_val
